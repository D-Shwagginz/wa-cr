...
# TODO: Double check variable types
...
module Parse
  ...
  def self.linedefs(wad : WAD, directory : WAD::Directory) : Array(Linedefs)
  ...
  def self.sidedefs(wad : WAD, directory : WAD::Directory) : Array(Sidedefs)
  ...
  def self.vertexes(wad : WAD, directory : WAD::Directory) : Array(Vertexes)
  ...
  def self.segs(wad : WAD, directory : WAD::Directory) : Array(Segs)
    parsed_segs = [] of Segs
    segs_index = 0
    File.open(wad.filename) do |file|
      file.read_at(directory.file_pos, directory.size) do |io|
  ...
  def self.ssectors(wad : WAD, directory : WAD::Directory) : Array(Ssectors)
  ...
  def self.nodes(wad : WAD, directory : WAD::Directory) : Array(Nodes)
  ...
  def self.sectors(wad : WAD, directory : WAD::Directory) : Array(Sectors)
  ...
  def self.reject(wad : WAD, directory : WAD::Directory) : Reject
  ...
  # Parses a map given the map and wad
  def self.map(wad : WAD, map : WAD::Map) : ParsedMap
    parsed_map = ParsedMap.new
    parsed_map.name = map.name
    parsed_map.things = things(wad, map.things)
    parsed_map.linedefs = linedefs(wad, map.linedefs)
    parsed_map.sidedefs = sidedefs(wad, map.sidedefs)
    parsed_map.vertexes = vertexes(wad, map.vertexes)
    parsed_map.segs = segs(wad, map.segs)
    parsed_map.ssectors = ssectors(wad, map.ssectors)
    parsed_map.nodes = nodes(wad, map.nodes)
    parsed_map.sectors = sectors(wad, map.sectors)
    parsed_map
  end

  # Parses a WAD given a WAD object, not path
  def self.wad(wad : WAD) : ParsedWAD
    parsed_wad = ParsedWAD.new
    wad.maps.each do |mapdir|
      parsed_wad.maps << map(wad, mapdir)
    end
    parsed_wad
  end
end
